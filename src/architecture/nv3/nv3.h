/* 
    ...
    nv3.h: NV3/NV3T (RIVA 128/128ZX) programming environment - NVPLAYGROUND ONLY, NOT API
*/
...
typedef struct nv3_object_class_001
{
    ...
    uint8_t reserved3[0x1CFC];                          // needs to be 0x2000 bytes 
...
typedef struct nv3_object_class_003
{
    ...
    uint32_t set_notify;                                // Set notifier
    uint8_t reserved2[0x1F8];
    uint32_t color;                                     // Color key value
    uint8_t reserved3[0x1CFC];                          // needs to be 0x2000 bytes     
} nv3_chroma_key_t;
...
typedef struct nv3_object_class_004
{
    ...
    uint32_t set_notify;                                // Set notifier
    uint8_t reserved2[0x1F8];
    uint32_t color;                                     // Color to use for plane mask
    uint8_t reserved3[0x1CFC];                          // needs to be 0x2000 bytes     
} nv3_plane_mask_t;
...
typedef struct nv3_object_class_006
{
    ...
    uint32_t color1;                                    // 0 bits =color0, 1 bits =color1
    uint32_t pattern[2];                                // 0 bits =c olor0, 1 bits color1
    uint8_t reserved4[0x1CE0];                          // needs to be 0x2000 bytes     
} nv3_pattern_t;
...
typedef struct nv3_object_class_007
{
    ...
    nv3_coord_16_t position[16];
    nv3_coord_16_t size[16];
    uint8_t reserved4[0x1B80];
} nv3_rectangle_t;
...
typedef struct nv3_object_class_008
{
    ...
    uint32_t color;                                         // use nv3_color_expanded_t but changed for alignment reasons
    uint8_t reserved3[0xF8];                  
    nv3_coord_16_t point[32];                               // Boring points 
...
typedef struct nv3_object_class_00B
{
    ...
    // The 16-bit points of the triangle.
    nv3_coord_16_t points[3];
    
    uint32_t reserved4;
    // Another 32-bit way of filling out the points of the triangle
    uint32_t x0;
    uint32_t y0;
    uint32_t x1;
    uint32_t y1;
...
{ 
    ...
    uint32_t line_count;
    uint32_t format;                                                // input increment 1 2 or 4, output increment 1 2 or 4 (represented by << 8)
    uint32_t buffer_notify;                                         // buffer for nvnotifications
    uint8_t reserved4[0x1CD4];  
...
typedef struct nv3_object_class_00E
{
    ...
    uint32_t pitch;                                                 // also pitch_420           4e0404
    uint32_t offset;                                                // also offset_y if yuv     4e0408
    uint32_t point;                                                 // also offset_u if yuv     4e040c
    // YUV420 stuff
    uint32_t offset_v;                                              //                          4e0410
    uint32_t point_y;                                               //                          4e0414
    uint8_t reserved4[0x1BE8];  // pad to 0x2000
...
typedef struct nv3_object_class_011
{
    ...
    uint32_t color[32];                                             // use nv3_color_expanded_t but changed for alignment reasons
    uint8_t reserved4[0x1B80];
} nv3_image_t;
...
typedef struct nv3_object_class_012
{
    ...
    uint32_t set_notify;
    uint8_t reserved2[0x200];
    uint32_t color_0;                                               // colour for 0 bits; use nv3_color_expanded_t but changed for alignment reasons
    uint32_t color_1;                                               // colour for 1 bits; use nv3_color_expanded_t but changed for alignment reasons
...
/* 
    ...
    NOTE: 2048X2048 IS A RIDICULOUS SIZE
*/
...
/* 
    ...
    Direct 3D 5.0 accelerated triangle
    Seems to be, by the very large color array, the main class used in 2d acceleration.
*/
typedef struct nv3_object_class_017
{
    ...
    uint32_t fog_color;                                     // Alpha is ignored here!
...
{
    uint32_t color;                                             // use nv3_color_expanded_t but changed for alignment reasons
    uint32_t zeta;                                              // 16 bits z, 8 bits stencil
} nv3_zeta_buffer_t;

/* 
    Object Class 0x18 (real hardware)
    0x?? (drivers)
    Also 0x58 in context IDs.
    
    Point but with the d3d zeta buffer
*/
...
/* 
    Object Class 0x1C (real hardware)
    0x?? (drivers)
    Also 0x58 in context IDs.
    
    Buffer settings, which buffer you are setting up is based on the GROBJ 
*/
typedef struct nv3_object_class_01C
{
    ...
    uint32_t linear_address;                            // 22-bit: Linear address in vram for the buffer position.
...
typedef struct nv3_subchannel_s
{
    union  
    {
        ...
        struct nv3_object_class_00C gdi; 
        struct nv3_object_class_00D m2mf;
        struct nv3_object_class_00E scaled_image_in_memory;
        struct nv3_object_class_010 blit;
        struct nv3_object_class_011 image_from_cpu;       
        struct nv3_object_class_012 bitmap; 
        struct nv3_object_class_014 image_to_memory;
        struct nv3_object_class_015 stretched_image_from_cpu;
        struct nv3_object_class_017 d3d_triangle;
        struct nv3_object_class_018 point_zeta;
        struct nv3_object_class_01C buffer_settings;
    };
    
} nv3_subchannel_t;

typedef struct nv3_channel_s
{
    nv3_subchannel_t subchannels[NV3_DMA_SUBCHANNELS_PER_CHANNEL];
} nv3_channel_t; 
/* Mode entry */
